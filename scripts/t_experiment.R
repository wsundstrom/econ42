...
subDir <- "/Google Drive/data/gerbergreene"
...
rm(mainDir1,mainDir2,subDir)

#install.packages(c("CausalGAM", "AER", "quantreg"))   
#install.packages("sm")
...
Y1 = b0 + D1*(Y0*10) + (1-D1)*Y0 + b1*x1 + b2*x2 + b3*x3 + err # Outcome in population when no compliance or attrition
...
varestATE1<-with(samp, sqrt( (var(Y1[D==1]))/(N/2) + (var(Y1[D==0]))/(N/2)))
